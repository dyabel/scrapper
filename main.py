...
class SemanticScholarScrapper(object):
    ...
    def scrap_paper_list_by_title(self, paper_title_list: list) -> dict:
        ...
        for paper_name in tqdm.tqdm(paper_title_list):
            try:
                paper_dict = self.scrap_paper_by_title(paper_name, call_browser=True)
    ...
    def _get_bibtex_citation(self) -> str:
        ...
        self._wait_element_by_class_name('bibtex-citation')

        bibtex_box = self._web_driver.find_element_by_class_name('bibtex-citation')
    ...
    def _open_first_link_in_search_page(self) -> None:
        ...
        self._wait_element_by_class_name('cl-paper-title')
        # self._wait_element_by_class_name('search-result-title')

        papers_div = self._web_driver.find_element_by_class_name('cl-paper-title')
        # papers_div = self._web_driver.find_element_by_class_name('search-result-title')
        first_paper_link = papers_div.find_element(by=By.TAG_NAME,value='span')

        first_paper_link.click()
        time.sleep(5)
        # print(self._web_driver.page_source,file=open('semantic.html','w'))
